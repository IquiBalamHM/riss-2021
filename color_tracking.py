...
class MaskClass:
  ...
  def process(self):
    ...
    for c in cnts:
      ...
      cv2.circle(current, (cX, cY), 7, (255, 255, 255), -1)
...
while cap.isOpened():
    ...
    maskgreen = MaskClass(frame_HSV.copy(),lowergreen,highergreen)
    maskgreen.process()
    cv2.imshow('green',maskgreen.tagged)
    maskyellow = MaskClass(frame_HSV.copy(),loweryellow,higheryellow)
    maskyellow.process()
    cv2.imshow('yellow',maskyellow.tagged)
    maskblue = MaskClass(frame_HSV.copy(),lowerblue,higherblue)
    maskblue.process()
    cv2.imshow('blue',maskblue.tagged)
    fullcenters = []
    fullcenters.append(maskblue.centers)
    #print(maskblue.centers)
    fullcenters.append(maskgreen.centers)
    fullcenters.append(maskyellow.centers)
    hand1 = HandClass(fullcenters)
    #print(len(fullcenters[0]))
    if hand1.numberofFingers == 3:
      print(hand1.area)


    masksum = maskgreen.tagged+maskyellow.tagged+maskblue.tagged
