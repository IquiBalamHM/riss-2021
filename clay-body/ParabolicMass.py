...
class DemoRope(App):
    ...
    def Initialize(self):
        ...
        for i in range(self.segments+1):
                    ...
                    if i != self.segments:
                        rope.AddParticles(
                            self.world.AddParticle(30+i * 20.0, self.world.hsize.y+10,Material(0.4,0.4,1.0)),
                            self.world.AddParticle(30+i * 20.0, self.world.hsize.y-10,Material(0.4,0.4,1.0)),
                    )
                    rope.AddParticles(
                            self.world.AddParticle(20+i * 20.0, self.world.hsize.y+10,Material(0.4,0.4,1.0)),
                            self.world.AddParticle(20+i * 20.0, self.world.hsize.y-10,Material(0.4,0.4,1.0))
                    )
                        # y=210.0
                        #self.world.AddParticle(5.0 + i * 10.0, 200.0)) # y=270.0
        for i in range(0, self.segments):
            rope.AddConstraints(self.world.AddConstraint(rope.particles[i*3],rope.particles[i*3+3],1.0),
                                #self.world.AddConstraint(rope.particles[i*3],rope.particles[i*3+1],1.0),
                                #self.world.AddConstraint(rope.particles[i*3],rope.particles[i*3+2],1.0),
                                #self.world.AddConstraint(rope.particles[i*3+1],rope.particles[i*3+3],1.0),
                                #self.world.AddConstraint(rope.particles[i*3+2],rope.particles[i*3+3],1.0),

            )
            if i != self.segments-1:
                rope.AddConstraints(
                #self.world.AddConstraint(rope.particles[i*3+1],rope.particles[i*3+4],1.0),
